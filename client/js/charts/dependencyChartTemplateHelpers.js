Template.flowerChart.rendered = function () {
    this.autorun(function () {
        ...
        if (currentApp.dependencies && currentApp.dependencies.length > 0) {
            ...
            var myFlower = new CodeFlower("#flowerChart", 400, 400);
        ...
        _.each(app.dependencies, function (dep) {
            if (dep) {
                if (_.find(apps, function (app) {
                        return app.name === dep.name;
                    })) {
                    imports.push(dep.name);
                }
            }
        });
        var input = {
...
Template.dependencyChart.rendered = function () {
    this.autorun(function () {
        var apps = Session.get('currentChartData');

        if (apps && apps.length > 0) {
            var chartInput = formatForChart(apps);
            renderDependencyChart(chartInput);
        }
    });
};

var renderDependencyChart = function (data) {
    var w = 880,
        h = 600,
        rx = w / 2,
        ry = h / 2,
    ...
    $("#dependencyChart").empty();

// Chrome 15 bug: <http://code.google.com/p/chromium/issues/detail?id=98951>
    var div = d3.select("#dependencyChart").insert("div", "h2")
        .style("left", "0px")
        .style("width", w + "px")
        .style("height", w + "px")
